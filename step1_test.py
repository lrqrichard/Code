...
csvfile = open('jan2june.csv','w',newline='')
...
for line in f:
	...
	if Coupon_id != 'null':
		...
		if dat < 7:
			writer.writerow({'User_id':User_id,'Merchant_id':Merchant_id,'Coupon_id':Coupon_id,'Discount_req':Discount_req,'Discount_rate':Discount_r,'Distance':Distance,'Date_received':Date_received,'Use':Use})
csvfile.close()
f.close()

csvfile = open('july.csv','w',newline='')
fieldnames = ['User_id','Merchant_id','Coupon_id','Discount_req','Discount_rate','Distance','Date_received','Use']
writer = csv.DictWriter(csvfile,fieldnames=fieldnames)
f = open('offline_test.csv','r')
for line in f:
	User_id,Merchant_id,Coupon_id,Discount_rate,Distance,Date_received = line.split(",")
	User_id = User_id.strip()
	Merchant_id = Merchant_id.strip()
	Coupon_id = Coupon_id.strip()
	Discount_rate = Discount_rate.strip()
